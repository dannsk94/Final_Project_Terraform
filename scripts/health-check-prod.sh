...
if curl -s -o /dev/null --max-time 10 http://$LB_IP; then
  ...
  openstack --os-cloud vkcs floating ip set --port "$(openstack --os-cloud vkcs loadbalancer show lab7-prod-lb -c vip_port_id -f value)" "$NEW_IP"
...
if ping -c 2 -W 3 "$BASTION_IP" > /dev/null 2>&1; then
  ...
  NEW_IP=$(openstack --os-cloud vkcs floating ip create internet -c floating_ip_address -f value)
  BASTION_PORT=$(openstack --os-cloud vkcs port list --server lab7-prod-bastion -c ID -f value | head -1)
