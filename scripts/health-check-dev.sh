...
if ping -c 2 -W 3 "$LB_IP" > /dev/null 2>&1; then
  ...
  OLD_IP=$(openstack floating ip list --fixed-ip-address "$(terraform output -raw load_balancer_ip)" -c "Floating IP Address" -f value)
  openstack floating ip delete "$OLD_IP"
  NEW_IP=$(openstack floating ip create internet -c floating_ip_address -f value)
  openstack floating ip set --port "$(openstack loadbalancer show lab7-dev-lb -c vip_port_id -f value)" "$NEW_IP"
  echo "Новый Floating IP балансировщика: $NEW_IP"
fi

# Проверка бастиона (dev)
...
if ping -c 2 -W 3 "$BASTION_IP" > /dev/null 2>&1; then
  ...
  OLD_IP=$(openstack floating ip list --fixed-ip-address "$(terraform output -raw bastion_public_ip)" -c "Floating IP Address" -f value)
  openstack floating ip delete "$OLD_IP"
  NEW_IP=$(openstack floating ip create internet -c floating_ip_address -f value)
  BASTION_PORT=$(openstack port list --server lab7-dev-bastion -c ID -f value | head -1)
